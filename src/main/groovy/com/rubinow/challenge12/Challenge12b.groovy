...
// expected result is 140718

new ChallengeB().go('/ch12/12.data');
...
class ChallengeB {
    ...
    def Set<String> getPossibleMoves(Map<String, Set<String>> cave, List<String> pathSoFar) {
        ...
        Set<String> cantRevisit = pathSoFar.stream().filter( location -> {
            location == 'start' ||
                    (location == location.toLowerCase() && haveRevisitedALowerCase(pathSoFar))
    ...
    def haveRevisitedALowerCase(List<String> path) {
        Set<String> counter = new HashSet<>()
        Iterator<String> lcs = path.stream().filter(location -> location == location.toLowerCase()).collect(Collectors.toList()).iterator()

        while(lcs.hasNext()) {
            String room = lcs.next()
            if( counter.contains(room)) {
                return true
            } else {
                counter.add(room)
            }
